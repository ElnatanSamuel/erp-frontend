...
export default function CreatePayslipPage() {
  ...
  useEffect(() => {
    // users (use backend API util so auth/base URL apply)
    (async () => {
      try {
        ...
        // keep silent; UI already shows fallback
      }
    })();
    // salary definitions
  ...
  async function onCreate() {
    ...
    setSubmitting(true);
    try {
      const created = await api<{ id: string }>('/payroll/payslips', {
        method: 'POST',
        body: JSON.stringify({
          staffName,
          title,
      ...
      router.push(created?.id ? `/payroll/payslips/${created.id}` : '/payroll?tab=payslips');
    } catch (e: any) {
      setError(e?.message || 'Failed to create payslip');
    } finally {
      setSubmitting(false);
    }
  }
    ...
    <div className="h-screen overflow-hidden bg-[#F6F8FB] flex">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <Card className="p-6 lg:p-7">
          <h3 className="text-base font-semibold text-gray-900">Create Payslip</h3>
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-5">
            <div>
              ...
              {staffOptions.length === 0 && <span className="text-xs text-rose-600">No staff found</span>}
            </div>
            <div>
              ...
              {titles.length === 0 && <span className="text-xs text-rose-600">No titles found</span>}
            </div>
            <div>
              ...
              {levels.length === 0 && <span className="text-xs text-rose-600">No levels found</span>}
            </div>
          </div>
          </Card>
        ...
        <Card className="p-6 lg:p-7">
          ...
          <div className="mt-6">
            <button onClick={onCreate} disabled={submitting} className="inline-flex items-center justify-center h-11 px-8 rounded-xl text-white bg-gradient-to-r from-[#1D75FF] via-[#5B6EF5] to-[#1B57E9] hover:brightness-105 shadow-sm disabled:opacity-50">{submitting ? 'Creating...' : 'Create Payslip'}</button>
